...
README.md           Diese Datei — Aufbau, offene Punkte und der Verlauf
```
...
jüngste Eintrag steht oben. **Die Einträge sind das Protokoll, nicht die
Regel:** Was dauerhaft gilt, steht in `CLAUDE.md`, und wie eine einzelne Regel
entstanden ist, in `docs/regelhistorie.md`.

Bis zum 06.09.2026 stand dieser Verlauf in einer eigenen `AUFGABEN.md` mit
Kästchen zum Abhaken. **Eine Aufgabenliste, in der 24 von 50 Punkten erledigt
...
### Nachtrag vom 06.09.2026 — Die untere Längenklasse belegt sich nicht von selbst
...
- `npm run wochenvorschlag` zeigt die geschätzte Länge je Short und die
...
- Drei kurze Wechselreden geschrieben, je rund 52 Sekunden:
...
- Ein Fehlalarm der Kaltstart-Wache behoben: `sachwoerter` erkennt
...
### Nachtrag vom 06.09.2026 — Was der Längenvergleich noch nicht kann
...
### Nachtrag vom 06.09.2026 — Zwei neue Formate im Vorrat
...
- Die Fundstelle `tausende-computer-ddos` trägt jetzt den ganzen Satz mit
...
### Nachtrag vom 06.09.2026 — Die Wortwechselposen repariert, und der Vorrat war nie das Problem
...
- **27 Shorts standen auf einer verbotenen Pose**, 26 davon auf
...
- **Der Wochenvorschlag findet wieder eine Woche.** Er meldete zwei Tage
...
- **`wochenAuswaehlen` sortiert die knappe Bauform nach vorn.** Je Format
...
- **Zwei neue Wechselreden**: `lichtgeschwindigkeit-festgelegt` (PTB, der
...
### Nachtrag vom 06.09.2026 — Die Sonntagszeiten vorgezogen
...
- `kanalwoche` von 11:30 auf **9:00**, `wochenlauf` von 12:00 auf
  **10:00**
- **Und `rueckblick` von 9:30 auf 8:30** — er füllt die Datei, die die
...
- Alle drei `.plist` im Repo und in `~/Library/LaunchAgents` gleichgezogen,
  Dienste neu geladen

### Nachtrag vom 05.09.2026 — Der Stand, mittags
...
- **Warum die Dienste nie von selbst liefen, ist geklärt.** `pmset -g log`
...
- **Der Mac wird bewusst nicht geweckt** — Emirhans Entscheidung. Ein
...
- **Der Sonntagslauf steht auf 12:00, nicht 12:07.** Die alte Minute war
  ...
  ist keine Ausnahme**
- `skripte/wochenlauf.plist` liegt jetzt im Repo, wie die drei anderen.
  Er war der einzige Dienst, den ein frischer Klon nicht gekannt hätte
- **Der Vorrat trägt jetzt zwei Wochen.** 23 Zitatkarten, 4 Stationen,
...
- **„Es war einmal" hat wieder einen Entwurf.** `bildschirmschoner` —
  ...
  den dunklen Modus
- **Dazu `handy-tankstelle`** als `werhatrecht`: Beide Lager liegen
...
- **Sechs Commits gepusht** (`e4ccbff..f162ec3`). Es waren sechs, nicht sieben — die Zahl stand hier zweimal falsch
...
### Nachtrag vom 05.09.2026 — Die Kanalwoche
...
- **`jeKanal` kam nie beim Leser an.** `src/rueckschau.ts` beschreibt die
...
- **Die Form steht jetzt an einer Stelle.** `Kanalmessung` als Zod-Schema
...
- **`npm run kanalwoche`** → `src/kanalseite.ts`, erzeugt `kanalwoche.html`
  wie `belege.html`. Nordstern, Aufrufe je Kanal, Verlauf, jedes Video mit
  den drei Kanälen nebeneinander
- **`de.ganzakkurat.kanalwoche`**, sonntags 11:30 — vor dem Wochenlauf um
...
- `try/catch` je Video um die beiden Analytics-Aufrufe. Ein Fehlschlag
  ...
  ihrer
- **`skripte/rueckblick.plist` und `nachlegen.plist` zeigten auf
...
- **Warum der Dienst nie von selbst lief.** Geklärt am 06.09.2026, und
...
### Nachtrag vom 05.09.2026 — Sendestruktur und Sonntagsautomatik

- **Recherche:** TikTok 18–20 Uhr (Samstag stärkster Tag), Instagram 20
...
- **Instagram und TikTok müssen nicht angebunden werden** — Buffer liefert
...
- `SENDEPLAETZE`, `UHRZEIT_JE_DIENST`, `wochenAuswaehlen`,
  `npm run wochenvorschlag`, `--auswahl=automatisch`
- **Der Sonntagsdienst `de.ganzakkurat.wochenlauf`** läuft sonntags 12:07
  und stellt die fünf Videos der Folgewoche zur Freigabe bereit. Kette
  einmal vollständig getestet mit `--trocken`
- 45 Schlussposen getauscht; `npm run pruefen` meldet keinen
  `bildvielfalt`-Hinweis mehr
- **Ein Fehler von gestern repariert:** Beim Aufräumen gingen fünf
...
- **Der Vorrat trägt genau eine Woche.** Erledigt am 06.09.2026: 34
...
- **Vier Shorts der ersten Auswahl liegen bei 69–78 Sekunden.** Die
...
### Nachtrag vom 04.09.2026 — Der Projektordner

- **526 MB `laeufe/` auf 68 MB.** Die Videodateien der Läufe vom 25.08.,
...
- `tmp/standbilder` (21 MB), `graphify-out/` (3 MB), `belege.html` und die
  drei berechneten Probenordner (Raun, Swisch, Vorhang) — alle
  reproduzierbar
- **Nicht angetastet: `public/ton/` (41 MB) und die drei Probenordner mit
  ElevenLabs-Material.** Bezahlt und nicht deterministisch reproduzierbar —
  dieselbe Asymmetrie wie überall im Projekt
- **`npm run vorspannton` fehlte in `package.json`**, obwohl CLAUDE.md es
  ...
  zurückgeholt
- **Die 21 zugekauften Skills gelöscht.** Gemessen wurde, welcher fremde
...
- `skills-lock.json` vorher vervollständigt: sieben Skills standen nie
  darin, obwohl sie installiert waren. **Ein Rückweg, den man erst beim
  Löschen vermisst, ist keiner**
- **31 MB Ton gelöscht**, dessen Text es nicht mehr gibt. Gemessen je Lauf,
  ...
  Kaltstart
- **Fünf Laufordner gelöscht, die nichts mehr liefern.** `herkuenfteLesen`
  ...
  vorher**
- Die drei ElevenLabs-Probenordner und 44 props-Kopien je Dienst
- **Der einstimmige Bau ist aus dem Renderer raus.** `Sprechblase.tsx`
...
- **Gegenprobe am Bild:** Standbild aus dem gesendeten Video gegen ein
...
- **Die drei „Vorschauentwürfe" vom 02.09.** waren keine: Buffer hatte sie
...
- **`UNTERTITEL_ZONE` ist gefallen — und meine Vorsicht war falsch.** Ich
...
- **Gewirkt hat sie nur noch dort, wo keine Tonspur vorliegt**, und dort
...
- Mit ihr gegangen: `BUEHNE.hoeheOhneUntertitel` (die zweite Höhe),
...
- **Die Struktur steht** (05.09.2026): fünf Videos je Woche auf Mo, Mi, Fr,
  Sa, So um 18:00, Instagram um 20:00. Vier Rubriken je Woche, die
  vorratsstärkste zweimal — keine Zuordnung zu Wochentagen

### Nachtrag vom 04.09.2026 — Der Dialog bekommt einen zweiten Leser
...
- `npm run dialogprobe` gebaut und an Emirhans sechs Dialogen geeicht —
  es misst Watti 41,3 Zeichen, die Handzählung vom 02.09. hatte 41
- Die Verbheuristik ist **zweimal gescheitert** (26 und 4 Fehlalarme im
  Eichmaterial) und aus dem Zählwerk geflogen — sie geht an den Prüfer
- `.claude/agents/dialogpruefer.md`, zwölf Muster, jedes mit Nachweis
- **18 von 53 Entwürfen brachen das Schema**, `npm run pruefen` war
  grün — 16 behoben, die Sichtbarkeit in `schemapruefung.ts` repariert
- Drei widersprüchliche Zahlen zusammengeführt: Kaltstart 4,0 → 5,2
  (CLAUDE.md), Aufschlag 3,5 → 9 (`voice.md`), Pool-Stand 38 → 48 (Skill)
- Die Zeichenzahl im Skill war falsch: „65 gegen 35" steht in keiner
  Messung. Gemessen sind Watti 41 / Volti 50 bei ihm, 38 / 47 bei mir
- Ein Nummernkreis: `S1`–`S8`, `1`–`108`, `LA`–`LI`. Vorher gab es
  Befund 1 und 8 zweimal
- Vierzehn Befunde aus dem Szenarienordner nachgetragen (95–108)
- Die elf Doppelungsgruppen und die acht Widersprüche stehen als Tabelle
  am Ende der Analyse
- **Die Gegenprobe des Dialogprüfers ausgewertet.** Sieben von zwölf
  ...
  Befunde 109–111
- **Die Selbstprüfung im Skill gekürzt** — von acht Punkten auf fünf.
...
- **Der Prüferlauf über die neun Dialoge vom 04.09.**: elf Funde an
...
- Die zwei Schemabrüche in Emirhans Dialogen behoben, die Februarzeile
  und der Titel „Ein Passwort für alles" geändert
- Alle zehn Szenariendokumente neu gezogen — sie trugen veralteten
  Wortlaut und die npm-Kopfzeilen mit im Text (Befund 108, an mir selbst)
- **Die acht Widersprüche geprüft — keiner war einer.** Drei lösten sich
...
- Die Kaltstartkette steht: welches Wort (2) — wo im Satz (68) — ob
  bekannt (91). Befund 68 ist jetzt ein Satzbaubefund, 91 gilt nur
  unbekannten Wörtern
- **Zwei Schemabrüche in Emirhans Dialogen.** Erledigt: Beide Shorts
  werfen keine Fehler mehr, nur noch Hinweise. Die Regeln sind gewandert,
  nicht die Zeilen — wie vorgesehen
- **Acht Widersprüche in der Analyse** — aufgelöst und in
...
- **Die zehn Dialoge sind noch nicht vertont.** Erledigt: `laeufe/` hält
  Tonspuren für die Dialoge des Gegentests; mehrere davon sind gesendet
- **Die Vorbilder** — gestrichen am 06.09.2026 auf Ansage: *„Ich denke
...
- **Eine Quelle für `apps-schliessen`** — am 06.09.2026 gesucht und
...
- **Der dreizehnte Bogen** steht als Idee `wechseln-ohne-verdacht` im
...
- **`memory-effekt` fehlt im Ideenvorrat.** Am 06.09.2026 geprüft und
...
- **`werhatrecht` verlangt `irrtum: 'beide'`.** Die Frage ist am
  ...
  richtig."
- **Der Zeiger ist ausgebaut**, beide: der Gefällt-mir-Zeiger mitten im
...
- **Eine Videodatei je Short statt drei.** `wochenlauf.ts`,
  `veroeffentlichen.ts` und die Freigabeseite kennen keine Fassungen mehr
- **`npm run bildrand` prüft wieder beide Kanten in jeder Szene** — die
  Ausnahme galt nur dem Zeiger
- **Der Vorhang bekommt beim Zufahren einen Ton.** `schliessung`: der
  Hauch abwärts (1700 → 700 Hz) statt aufwärts, längerer Anstieg am
  Grundton, 0,62 s statt 0,96, gemessener Ziel-RMS 0,019
- **Befund 117:** Der Name des anderen als eigene Zeile am Schluss ist
  Enttäuschung, im Satz eine normale Anrede. Muster im `dialogpruefer`
- **Wattis Hand am linken Bildrand gefunden und gesperrt.** Ursache:
...
- **Neu gerendert mit `--ton-behalten`**, 993 Standbilder geprüft,
  Freigabe erteilt
- **Die vier Videos sind eingeplant**, zwölf Beiträge über drei Kanäle:
...
### Nachtrag vom 03.09.2026 — Durchgang 2 der Beispieldialoge
...
- **Sechs neue Quellen**, 21 neue Fundstellen: ESA und University of
...
- **Elf Entwürfe** in `daten/entwuerfe/`, `GEPARKT` steht bei 28
- **`npm run szenarienblock`** zieht die Dialogblöcke aus den Entwürfen.
...
- **Neun Ideen auf `belegt`** mit ihren Quellen
...
- In drei bis vier Wochen wiederkommen: 15 Videos draußen, 11 mit
  Haltekurve. Die Aussage steht, und sie hat den Nordstern gewechselt

### Nachtrag vom 02.09.2026 — Der Skriptschreiber
...
- **Neun Dialoge vermessen** — 131 Redezeilen, Zeichen je Figur,
  Fragenanteil, Sitz der Zitatkarte. 38 Befunde in
  `daten/marke/dialoganalyse.md`
- **Sechs Runden Gegentest.** Runde vier, 5c und 6a waren die ersten ohne
  Beanstandung
- **`daten/szenarien/`** mit LIESMICH und fünf Szenariendokumenten. Der
  Skill liest den Ordner, er enthält ihn nicht — **eine neue Runde legt
  dort eine Datei ab und fasst den Skill nicht an**
- **Fenster 40–80**, Zielwerte je Bauform gestrichen,
  `LAENGENKLASSEN` aus dem Fenster abgeleitet
- **`MACHARTEN` mit Feld `wer`**, sechzehn Einträge statt sechs. Voltis
  Fach hatte im Schema gar keinen Platz
- **Die Beschreibung** lädt ein statt aufzuzählen, und `beitragstext`
  sammelt die Quellen aus Szenen **und** Redeanteilen
- **Alle zehn Dialoge im Schema**, in `GEPARKT`, 54 bis 78 Sekunden
- **Die Regelinventur.** 91 Befunde an zehn abgenommenen Dialogen; neun
  ...
  `erinnern`
- **`npm run pruefen` sieht `GEPARKT`.** Zehn fertige Dialoge lagen im
  Ordner, die Prüfung war grün, und 91 Befunde standen daneben
- **Der `belegpruefer` hat einen Short umgedreht.** In
...
- **Zwei Quellen dazu:** § 434 BGB und zwei weitere Zitate der
  BSI-Löschseite
- **`skript-schreiben`, `voice.md` und `CLAUDE.md`** auf den Stand

#### Offen

- **Die Szenarien 6 bis 12 brauchen Beispiele.** Alle zwölf haben jetzt
  welche — Durchgang 2 hat elf Dialoge gelegt, zwei davon Emirhans eigene
  aus `daten/briefings/`
- **Durchgang 3 und 4: je Szenario zwei weitere Beispiele.** 48 von 48
  stehen, alle zwölf Szenarien voll — 04.09.2026

### Nachtrag vom 02.09.2026 — Die Kanalbilder holen den Show-Look nach
...
- **Der Banner ist eine Bühne.** `Vorhangstoff` aus dem Video, mit einer
  Bahn von 480 Pixeln je Seite. Kein zweiter Vorhang von Hand
- **Der Akzent ist Theaterrot statt Marineblau** — im Muster und im
  Strich unter dem Spruch. Blau gehört seit dem 31.08. einer der beiden
  Figuren, nicht dem Kanal
- **Die Figuren stehen auf 186 statt 130 Pixeln**, der Satz fällt dafür
  von 72 auf 56
- Die zweifarbige Wortmarke aus der Kopfzeile steht zwischen Figuren und
  Satz
...
- **Die Wortmarke im Banner.** Beide Fassungen liegen in `marke/`, die
  ...
  hochgeladen**
- **Die Anschlussregel gefallen.** Sie verlangte, dass die erste Zeile
...
- **Die Kaltstart-Grenze von 3,5 auf 4,0 Sekunden.** Seine erste Zeile
...
- **`ZUGRAUM` rechnet die Kombinatorik**, statt sie danebenzuschreiben.
  `bitten` hat 72/380/2.600 still falsch gemacht
- Sein Dialog steht in `passwort-wechseln`, 60,5 Sekunden geschätzt. Die
  Wachen haben zwölfmal gemeldet und zwölfmal recht gehabt
- **Zwei Skills:** `thema-briefen` und `humor-eichen`
- **Zehn Briefingbogen** unter `daten/briefings/`, alle auf geprüften
  Quellen
...
### Nachtrag vom 02.09.2026 — Der Kaltstart vor dem Vorhang
...
- **`kaltstart` als Feld am Short**, mit `KALTSTART_ARTEN` (sechs für
...
- **Der Vorhang fährt wieder zu.** Der Grund gegen das Zufahren vom
  31.08. war die leere Bühne bei Bild 0 — jetzt steht dort der Kaltstart
- **Showtitel und Einwurf gestrichen**, gesprochen wie geschrieben. Auf
  der Karte steht fest „Die Volti & Watti Show", die Namen in ihren
  Kennfarben. Zehn feste Aufnahmen entfallen
- **Die Themenzeile trägt einen Namen und behauptet nichts mehr.**
...
- **Anschlussregeln:** andere Figur, antwortender Zug, Wortbrücke als
  Hinweis; laufweit keine Aufbauart zweimal hintereinander
- **Der Skill `skript-schreiben`** — der fehlende Schritt zwischen
  `thema-finden` und `woche-bauen`
- Alle vier Entwürfe angepasst, `npm run pruefen` und `npm run bildrand`
...
- **Die Dialoge sind nur mechanisch angepasst.** Beschreibt den Stand
  ...
  vorsah**
- **Die Wortbrücke bei `passwort-wechseln` meldet.** Der Befund stimmt
  ...
  nicht für diesen
- **Kein Ton für den Kaltstart.** Erledigt: `src/stimme.ts`
  synthetisiert ihn als eigenen Aufruf mit der Stimme, die
  `KALTSTART_SPRECHER` für das Format vorsieht
- **`daten/vorspannton.json` hält noch die zehn Formatdauern.** Sie
...
### Nachtrag vom 02.09.2026 — Vorschau in Buffer
...
- Aufgeräumt: drei Beiträge in den Apps gelöscht, `probe/` in R2 geleert,
...
- **Die drei Fassungen je Dienst sind byte-identisch.** Entschieden am
...
- **Die Tonspur sagte in Szene 2 noch „fremdes Gerät"**, der Entwurf „Von
  einem fremden Gerät". Erledigt mit dem Lauf vom 04.09.

### Nachtrag vom 01.09.2026 — Die vierte Wand steht — und hat zwei Zeichenfehler mitgebracht

- Pose `ansprechen` plus Posenfaktor `zuwendung`, `Zuwendungsprobe` mit
  `Sprecherstand` und Gegenprobe. Alle vier Nachschläge tragen sie jetzt
- **Die Armvorzeichen sind seitenabhängig.** Mit −10/−28 auf beiden
  Seiten klappte nur der linke Arm an den Bauch. Am Standbild gelernt,
  nicht am Code
- **`nachdenken` lag quer über dem Mund.** Gefunden beim Durchsehen aller
  elf Posen zu zweit — die Pose war seit dem 23.08.2026 im Vokabular und
  **in allen vier Entwürfen benutzt**. Unterarm 60 → 38

- **Die Figuren im Schluss größer.** `WORTWECHSEL_SCHLUSS` mit 0,92 statt
...
### Nachtrag vom 01.09.2026 — Die Gegenprobe fand elf weitere — darunter die dokumentierte Falle
...
### Nachtrag vom 01.09.2026 — Der Belegprüfer — elf Stellen, eine davon umgekehrt
...
### Nachtrag vom 01.09.2026 — Die Karte zurück

- **Opener und Abspann wieder in der abgenommenen Geometrie** — Titel
  132, Abstände, Figuren unten. Ich hatte sie für die Geisterfiguren
  umgebaut, ohne dass jemand darum gebeten hatte
- **Die Geister behebt ein Zeitpunkt:** Die Abspannkarte blendet erst ein,
...
### Nachtrag vom 01.09.2026 — „Wirklich."

- **Der Abspann ist ein fester Zweizeiler**: Volti „Wir haben
  nachgelesen.", Watti „Wirklich." Zwei Aufnahmen, einmalig 93 Zeichen
  (31 je Versuch, dreimal). Das Feld `abspann` ist wieder weg
- **„Von einem fremden Gerät."** statt „Fremdes Gerät." — kostet beim
  nächsten Lauf die Vertonung dieser Szene
- **Die Redespalten schnitten ab**, wo vorher die Zitatkarte endete:
  `overflow: hidden` an der sicheren Zone. Jetzt Umbruchzeilen schätzen,
  ganze Gruppen verwerfen
- `NACHLAUF_SEK` aus den gemessenen Abspanndauern gerechnet, 3,5 statt
...
### Nachtrag vom 01.09.2026 — Abspann wie der Opener, Pausen, Vorlauf

Vier Ansagen zum Handy-Video.

- **Der Abspann sieht aus wie der Opener** — Showtitel und Namen bleiben,
  nur die Mitte wechselt: „Wir haben nachgelesen." und Wattis Zeile. Der
  Schlusssatz steht nirgends mehr im Bild
- **Die Geisterfiguren.** Die Kartenfiguren standen 500 Pixel tiefer als
  ...
  in der Bühnenfläche
- **Pausen 0,15 → 0,45 und 0,2 → 0,7**, ohne neue Vertonung:
  `tonspurNeuLegen` legt die bezahlte Tonspur mit den aktuellen Pausen neu.
  49,9 → 56,7 Sekunden
- **Volti wartet 0,6 Sekunden** vor dem Showtitel
...
- **Die Zitatkarte steht unten**, vor den Figuren, in 0,72 der
  Schriftgröße — in voller Größe deckte sie die beiden bis zum Kopf
- **Die Redespalten** sammeln die Zeilen der laufenden Szene unter der
  Figur, die sie sagt, in ihrer Farbe
- **Der Abspann auf dem Vorhang** — „WIR HABEN NACHGELESEN", der
...
### Nachtrag vom 01.09.2026 — Die Kulisse
...
- `text` und `hervorhebung` aus `SzeneText` gestrichen, 24 Zeilen aus den
  vier Entwürfen entfernt
- Die `Text`-Komponente im Renderer heißt `NurBuehne` und trägt nur noch
  die Zeichnung
- Der Suchbegriff-Hinweis auf den Bildtext ist ersatzlos weg — die beiden
  tragenden Drittel sind Sprechtext und Beschreibung, und die werden hart
  geprüft
- `Textschleier` hinter Schluss, Frage und Zahl
- `children` an `Buehne` ist optional
...
### Nachtrag vom 31.08.2026 — Der Ton

- **Blindwahl für die Regieanweisungen** — gestrichen am 06.09.2026 auf
  ...
  hier beschrieben
- **Beef umgebaut** — am 06.09.2026 auf Emirhans Entscheidung, gegen
...
- **`schaetzmal` als sechstes Format** — verworfen am 06.09.2026 auf
...
- **Eine neue Ideenquelle** — gefunden am 06.09.2026: **die vier Seiten
...
- **Vorrat nachfüllen unter sechs offenen Ideen** — keine Aufgabe,
...
- **`CLAUDE.md` auf altem Stand.** Über die Wochen abgearbeitet; die
...
- **`bild-bauen/SKILL.md`** nachgezogen: Er sagt jetzt am Anfang, welche
  Referenzdatei für welche Aufgabe zu lesen ist
- **Ein fünfter Skill für die Bebilderung?** Nicht nötig — erledigt am
  ...
  beantwortet**
- **`laeufe/` liegt bei 235 MB in einem einzigen Ordner.** Erledigt am
...
- **`daten/benchmarks.md` und `npm run nachfrage`.** Beides gibt es nicht
  mehr — die Datei fehlt, das Skript steht nicht in `package.json`. Beim
  Aufräumen am 04.09. mitgegangen
- Den Tagesstand pushen
- Die nächste Woche bauen — macht seit dem 06.09.2026 der Dienst
  `de.ganzakkurat.wochenlauf`, sonntags um 10:00
- **Die drei übrigen Entwürfe migrieren** — alle vier sind jetzt
...
- **`zug` auf Pflicht**, Übersprungzweig gelöscht, Quellensperre und
  ...
  Sperre vollständig
- **Die laufweite Tripelregel** in `laufweiteBefunde`, auf benachbarten
  Shorts. Sie hat beim ersten Lauf dreimal gemeldet und dreimal recht
  gehabt
- **Die vierte Wand**: gebaut. `ansprechen` steht in `src/figur.ts` mit
  `zuwendung: 0`, der Faktor skaliert Blick und Neigung, und
  `Zuwendungsprobe` prüft ihn samt Gegenprobe bei Bild 20
- **Die Haltungsprobe vor dem Bau.** Sie hat die Reihenfolge des Plans
...
- **Der Zug an der Tonspur** — `abschnitte[].zug` als Pflichtfeld,
...
- **Der Publikumston** am Kipppunkt — und er ist kein Publikum geworden.
...
- **Ein Video mit vollem Ton.** Erledigt — mehrere Läufe mit Ton sind
  seit dem 04.09.2026 draußen

### Nachtrag vom 31.08.2026 — Was am fertigen Video entschieden wird

- **Die Letterbox** — gestrichen am 06.09.2026 auf Ansage. Der Punkt war
...
- **Die Randbreite am Handy** — gestrichen am 06.09.2026 auf Ansage. Der
  Punkt sagte selbst, dass kein Standbild das entscheidet; entschieden hat
  es das gesendete Video
- **Die Formatpille in der Kopfzeile bleibt.** Entschieden am 06.09.2026
...
- **Facts und Kein Zufall sind beide blau.** Erledigt am 06.09.2026:
...
- **Das Logozeichen steht jetzt wie die Figur.** Entschieden und gebaut
...
- **Das Szenensymbol `batterie` liegt quer.** Bleibt liegen, entschieden
...
### Nachtrag vom 31.08.2026 — Was als Nächstes anstand

- **Den bezahlten Lauf abgesichert, bevor er läuft.** Drei Löcher, alle
...
- **Kurze Redeanteile vor dem Bezahlen.** Kein offener Punkt mehr,
  ...
  kein Lauf
- **Die Figuren sind je Szene verschieden groß.** Behoben: Die
  Figurenbühne steht absolut auf ihrer Standlinie und teilt sich die Höhe
  nicht mehr mit dem Text
- **Der Abspann ist stumm.** Zwei feste Aufnahmen, einmalig bezahlt
- **Die drei anderen Shorts auf Streit umschreiben.** Erledigt beim
  Dialogumbau: alle drei tragen `widersprechen`-Züge
- **`bild-bauen` kennt die Kulisse nicht.** Erledigt am 06.09.2026: Der
  Raum, der Schleier und die Überlaufmessung stehen in
  `references/buehne-und-kulisse.md`
- **Kein Budgetcheck vor dem Lauf.** Gebaut am 06.09.2026:
...
### Nachtrag vom 31.08.2026 — Das Bild der Gesprächsszenen

#### Der Freiraum — gemessen statt vermutet

- **Die Notiz stand falsch herum.** „Die Figuren sitzen tief, darüber
...
- **Zwei eigene Fehlrechnungen**, beide vor dem Bauen gefunden: Ein
...
- **`passwort-wechseln` zeigte in drei von sechs Szenen eine einzelne
  Figur**, während beide miteinander redeten. Die anderen drei Entwürfe
  waren sauber
- **Der Grund war der Weg des geringsten Widerstands:** Die drei Szenen
...
- **Neue Wache `zweiImBild`.** Sperren verbieten, sie verlangen nichts —
  dieselbe Lücke wie bei `zweistimmigkeit` und `reaktion`
- Gemessen: Die Bühnenzone ist jetzt zu **14,7 %** gefüllt statt zu 8,2 %
...
### Nachtrag vom 31.08.2026 — Zielwert und Fenster messen Verschiedenes

- **Der Bauform-Zielwert rechnet ohne den Vorspann.** Er sagt, wie lang
...
- **Anheben ging nicht:** 62 + 9,5 = 71,5 reißt die Obergrenze. Zwei
  Größen, die verschiedene Dinge meinen, brauchen verschiedene Zahlen
- `npm run pruefen` ist **vollständig grün** — kein Fehler, kein Hinweis,
...
### Nachtrag vom 31.08.2026 — Der Hauptteil wird ein Gespräch

#### Wattis Stimme — geklärt, nicht geändert

- **„Die Stimme von Watti ist nicht mehr die erste Stimme"** stand seit dem
...
- **Die Vereinbarung über „die erste Stimme" betraf Volti**, nicht Watti —
...
- Geändert hat sich nicht die Stimme, sondern **was sie spricht**: v3 statt
  v2, Regieanweisungen seit dem 26.08., die Stimmeinstellung
- Alle acht Kandidaten von damals gegen die heutige Aufnahme gehört.
  **Entschieden: es bleibt bei Prayan**

#### Der Gesprächsmaßstab

- **Emirhans Musterdialog steht wörtlich in `voice.md`.** Er stand bis
...
- Drei Merkmale, alle prüfbar: **Anrede**, **zweite Person**,
  **Rückbezug** auf ein Wort der Vorzeile
- `ersatzteil-freischalten` umgeschrieben: von **0 Anreden und 0 zweiter
  Person** auf 3 und 6, dazu drei Rückbezüge (Verbot, erlaubt, Drucker)
- **Die zweite Person hat zwei Grenzen sichtbar gemacht, die vorher
...
- Ein Zitat endete **vor seinem eigenen Verb** — „behindern" stand
  außerhalb der geprüften Zeichenkette, obwohl der Sprechtext es spricht
- Drei `stuetzt`-Zeilen nachgezogen. Sie ließen genau die Grenzen weg, die
...
### Nachtrag vom 31.08.2026 — Der Vorspann hat einen Ton

Fünf Klänge, zehn Aufnahmen, und der Vorhang ist an den Anfang gewandert.

#### Der Ton

- **Zehn feste Aufnahmen** je Format, 170 Zeichen einmalig. Showtitel und
  Namen wechseln nie; durch `shortVertonen` geschickt kostete derselbe Satz
  rund 11.000 Zeichen im Jahr
- **`eleven_v3` halluziniert bei kurzen Eingaben.** Fünf Läufe mit
...
- Das Skript läuft dreimal je Aufnahme und verwirft alles über vier
  Sekunden. **Sechs von dreißig Läufen fielen durch**
- **Die Themenansage** je Short, aus der Vertonung. Sie steht neben
  `abschnitte`, nicht darin — die Aufschlagmessung filtert gegen
  `szenenStartSek[1]`
- `VORSPANN_SEK` war eine Konstante und ist eine **Funktion**: Die
  Themenzeile wechselt je Short, eine Zahl für alle wäre für jeden die
  falsche
- **Ton und Bild aus einer Zahl.** `ansageAbBild` steuert beides; vorher
...
- **Stoffrauschen**: 67 % der Energie über 2 kHz — ein Fön
- **Applaus**: klang nach Knistern. Ein Hochpass lässt alles darüber
  durch; 5 % lagen im Klatschkörper statt 30
- **Swisch**: sanfter, allein aber fremd
...
- **Die Vorschaudatei spielte die Ansage doppelt.** Ich hatte dieselbe
...
- **Die zehn bezahlten Aufnahmen lagen nicht im Repo.** `*.mp3` ist
...
### Nachtrag vom 31.08.2026 — Aus Vortrag mit Zwischenruf wird ein Gespräch
...
- **`einstimmig` gestrichen.** Bei einer Untergrenze von 42 Sekunden ist
...
- **Zielwerte 45 / 52 / 62.** Drei von vier lagen unter dem Fenster — ein
  Zielwert, der den eigenen Short ungültig macht, ist eine Falle
- **Die Drittelregel wäre unerfüllbar geworden.** Bei drei Bauformen und
  sieben Shorts erlaubt ein Drittel höchstens sechs. Sie steht jetzt auf der
  Hälfte, wie beim Format

#### Die vier Entwürfe

- **Die Nähte lagen zwischen den Sätzen, nicht in ihnen.** Aufschlag 2,9 s
...
- **Watti trägt selbst einen Beleg** (`raumstation` Szene 6). Sechs
  Macharten je einmal reichen nicht, um 97 Zeichen aufzuholen
- Drei neue Sätze aus Fundstellen, die ungenutzt in den Quellen lagen.
  `stationen` hat damit erstmals vier Zuspitzungen
- **Die Längen streuen:** 61,5 / 49,6 / 48,1 / 47,3 s — drei Klassen statt
  einer. Der Versuch bis Oktober hat etwas zu messen
- Sechs Befunde des `belegpruefer` nachgezogen, **alle sechs in Sätzen, die
  gerade erst angefasst worden waren.** Jede Kürzung nimmt ein Wort mit, das
  gedeckt war

#### Die EU-Richtlinie war nie kaputt

- **Eine Wache, die an einer Schreibweise hängt, prüft die Schreibweise
...
- Beide Wege liefern byte-genau dieselbe Datei. Nachgemessen: 199.351
  Bytes, `cmp` meldet keinen Unterschied
- **Der Rechtsakt stand zweimal in `quellen.json`**, in beiden
...
- **Zwei Zitate sagten für sich gelesen das Gegenteil.** Subjekt und
  Verneinung stehen an den Enden des Satzes, die Aufzählung dazwischen
- Zitatlänge von 180 auf 240. Die 180 waren nie eine gemessene
...
### Nachtrag vom 31.08.2026 — Die Bühne steht von Anfang an
...
- **Randbreite 100 Pixel**, am Bild gewählt. Die Herleitung hätte 130
  ergeben (sichere Zone minus Reserve) — das war die größtmögliche Breite,
  nicht die richtige
- **50 Pixel wären unsichtbar gewesen.** Der am 15.08. an einem
...
### Nachtrag vom 31.08.2026 — Theaterrot im Renderer

Nach drei Vergleichsseiten sind alle Werte entschieden und eingebaut.

#### Farben

- **Theaterrot `#7E1F1F`** als erste Flächenfarbe der Marke. Rot war bei
  Wattis Farbwahl als „zu grell" verworfen worden — das galt einer **Figur**,
  nicht einer Fläche
- **Je Kennfarbe drei Helligkeiten**, mit gerechneter Begründung: Anzeige
...
- **Sechs Rubrikfarben**, alle ≥ 4,5 auf ihrer hellen Fläche. Pink und
  Orange mussten dafür nachgedunkelt werden (4,26 und 3,47)
- `anzeigeZweiHell` — das seit jeher fehlende Gegenstück zu `blauHell`

#### Kopfzeile

- **Doppelzeichen** statt eines einzelnen blauen Akkus: Volti ein Pol,
  Watti zwei. Versatz **78** — vorher 105, also 37 Einheiten Lücke bei 68
  Einheiten Gehäusebreite
- `Zeichenformen` herausgezogen, damit `Doppelzeichen` und `Logozeichen`
  dieselben Formen teilen statt sie abzuschreiben
- **Wortmarke zweifarbig** in den gedämpften Tönen `#303C6C` / `#896358`.
  Der erste Anlauf nahm die Anzeigefarben und wirkte flau — die sind für den
  dunklen Figurenkörper aufgehellt
- **Formatpille zurück**, mit dem Showtitel und der Rubrikfarbe
- Das Profilbild nutzt `Doppelzeichen` statt zweier Aufrufe von Hand — die
  Paarungsrechnung stand an zwei Orten

#### Vorhang

- **Falten mit Verlauf**, Breiten deterministisch gestreut (derselbe
  Generator wie das Bannermuster). Gleich breite Bänder wären ein
  Streifenmuster
- **Querbehang** mit Bögen und Quasten, bleibt oben hängen
- **Weißer Umriss** um die Figuren — **am selben Abend wieder entfernt.**
...
- **„HEUTIGES THEMA"** als feste Zeile im Renderer, Stärke 900 — außerhalb
  des geprüften Feldes, damit die Ansagesperre bestehen bleibt
- **„mit Volti und Watti"**, jeder Name in seiner aufgehellten Kennfarbe
- Typografie: Titel 132, Themenzeile 68–88 gestaffelt

#### Ein Fehler, im Standbild gefunden

- **Die Naht stand fest in der Bildmitte.** Bei halb geöffnetem Vorhang lag
...
### Nachtrag vom 31.08.2026 — Der Vorspann: aus vier Rubriken werden sechs Shows
...
- **`show`-Titel je Format** in `FORMATE`. Fünf gesetzt
- **Feld `vorspann`** am Short — die Themenzeile, das einzige, was je Short
  wechselt. Der Showtitel folgt aus `format`, der Ton ist eine feste Datei
- **Dieselbe Ansagesperre wie im Aufschlag** auf der Themenzeile, dazu
  „heute:" und „heutiges thema". Sie soll behaupten, nicht ankündigen
- Vier Themenzeilen geschrieben, jede eine Behauptung mit dem Suchbegriff
- **`VORSPANN_SEK` in `src/zeit.ts`.** Der ganze Zeitgriff sitzt an **einer**
...
- Gegengeprüft: Szene 1 spricht 3,26 s und dauert mit Vorspann 7,07 s;
  Szene 2 beginnt genau dort
- `video/bausteine/Vorhang.tsx` — zwei Hälften, Titelkarte, die beiden
  Figuren davor, gestaffelte Schriftgröße für die Themenzeile
- In `Short.tsx` eingehängt, Startbild **rückwärts** aus der Folgeszene
  gerechnet — dann wandert die Karte mit, wenn die Dauer sich ändert

#### Vier Fehler, alle erst im Standbild sichtbar

- **`Buehne` gibt ihren Kindern keine Höhe.** Sie misst den Inhalt und
  ...
  war null Pixel hoch
- **`[seite]: 0` setzte eine CSS-Eigenschaft namens `links`.** Bezeichner
  sind hier deutsch, CSS-Eigenschaften nicht — nur eine Hälfte stand da
- **`AbsoluteFill` überschreibt `right` und `bottom`** mit seinem
  `inset: 0`. `left` und `top` griffen, der Vorhang lief rechts aus dem
  Bild. Wer nur eine Seite prüft, sieht das nicht
- **Feste 52 px liefen über.** Die sichere Zone nimmt links 170 und rechts
...
- **Bauform `einstimmig` streichen** — Zielwert 25 s, Fenster ab 42 s
  *(erledigt 31.08. nachts)*
- **Die Bauform-Zielwerte gegen das Fenster 42–67 s.** 45 / 52 / 62 am
  31.08.2026 abends, `einstimmig` gestrichen. 25 / 35 / 45 / 60
  liegen zu drei Vierteln darunter

### Nachtrag vom 31.08.2026 — Korrektur am selben Abend
...
- **Die Pole sind getauscht.** Watti trägt die zwei (9-Volt-Block), Volti
  den einen — denselben, den auch das Logozeichen der Wortmarke zeigt
- **Die Stauchung ist ersatzlos gefallen.** Sie kam am 25.08. als
...
### Nachtrag vom 31.08.2026 — Volti umgebaut, Bewegungsstil in Produktion
...
- **Volti ist gestaucht wie Watti.** `ZEIGER_*` heißt `FIGUR_*` und steht
...
- Kantenrechnung symmetrisch, beide Schatten gleich breit
- **Anordnung 0,62 · 49/151.** Gerechnet ergab `staunen` als bindende
...
- **Ein Fund am Rand:** Der Kommentar behauptete, „beide greifen
...
- `poseAus` federt cartoonhaft, mit Overlapping und Anticipation als
...
- Gewichtsverlagerung und Atemvolumen dauerhaft, mit Versatz je Figur
- **Die Kamerafahrt ist verworfen, und zwar gerechnet:** Bei der laufenden
...
- **Stattdessen lehnt sich der Sprechende hin** (1,5°). Das passt in
...
- **Die Brauen sind in allen zehn Posen belegt.** Zehn Posen mit eigener
  Brauenhaltung sind zehn Gesichter statt einem — ohne ein Feld im Schema
  und ohne dass ein Entwurf angefasst wird
- Zwei Mundformen dazu: `spalt` und `weit`. Sieben statt fünf
- **Lippensync aus den Wortzeitstempeln.** Sie liegen seit dem ersten
...
- Silben aus **Vokalgruppen** des Wortes: „Ersatzteil" bekommt drei
...
- Satzschluss-Wörter tragen ihre Nachlaufstille im Zeitstempel (`passt.`
  = 720 ms bei einer Vokalgruppe). Die Öffnung sitzt deshalb am **Anfang**
  ihres Abschnitts und ist gedeckelt
- Ohne Tonspur bleibt der Sinus — jede Probe, jedes Standbild
...
### Nachtrag vom 31.08.2026 — Das Maximum des Figurenstils
...
- **`TEMPO.feder` hat nie gefedert.** `damping 200` bei einer kritischen
...
- **Die Beine waren tote Gelenke:** ±12° verfügbar, in keiner der zehn
  Posen benutzt
- **Die Brauen hatten kein Gelenk** — Teile ohne Beweglichkeit, und damit
  die größte ungenutzte Ausdrucksfläche der Figur
- **Drei Posen rissen ihre Gelenkgrenzen und wurden stumm abgeschnitten:**
...
- `AUSSENREICHWEITE` je Pose, aus der Gelenkkette gerechnet;
  ...
  gewesen**
- Anordnung von 0,75 · 43/149 auf **0,70 · 45/146**. Sieben Prozent
  Größe holen `staunen` (zehn Nennungen, die meistgenutzte Pose) und
  `hochschauen` zurück; die Lücke wächst dabei von 20,2 auf 20,9
- `achselzucken` bleibt im Wortwechsel gesperrt, und zwar geometrisch:
...
- Die Kamera folgt dem Sprecher — **verworfen**, und zwar gerechnet:
...
### Nachtrag vom 31.08.2026 — Das Bild zuerst
...
- **Die langen Pausen waren nicht die Sprecherpausen.** Die Wechsel lagen
...
- **Die Figuren wurden rechnerisch zwingend abgeschnitten.** 104
...
- `(Volti)` und `(Watti)` über den Figuren, im SVG statt in HTML. Beide
  dauerhaft, der Sprechende leuchtet
- `Sprecherstand.tsx` — wer spricht, als Context statt als Prop über vier
  Ebenen. Gerechnet auf der **absoluten** Zeitachse, weil `useCurrentFrame`
  in einer `Sequence` bei 0 beginnt
- **Die Sprechblase ist weg.** Bei zwei Figuren im Bild sah sie mit dem
  Untertitel nicht aus. Das Bauteil bleibt vollständig im Code
- Die Bühne bekommt die 270 Pixel Untertitelzone zurück, wenn kein
  ...
  wird
- **Sprechbewegung:** Blickkontakt zur Mitte, Mund im Sprechrhythmus,
  ...
  verschiedene Längen
- **`redelaeufe` schneidet an jeder Szenengrenze.** Kein ` ... ` mehr im
  ...
  nachjustieren lässt**
- `sprechtextZusammenfuegen` entfernt — die Funktion hat zuletzt nichts
  mehr vertont und zählte für `zeichenverbrauch` Trennerpunkte mit, die nie
  an die Synthese gingen. Jetzt über `redelaeufe`
- Die Pausenwache in `schemapruefung.ts` zählt jede Szenengrenze, nicht
  nur die mit Wechsel — und rechnet über `SZENENGRENZE_SEK` statt über eine
  abgeschriebene 0,32
- **Zwei fordernde Regeln:** `redelauf` (höchstens 6 s am Stück) und
...
- Längenfenster 20–65 → **42–67 s**
- `gibtswirklich` verliert den Freibrief „brauchen keine Pointe — die
...
### Nachtrag vom 26.08.2026 — Die erste fordernde Regel
...
- **Zweistimmigkeit**: mindestens zwei Szenen je Short tragen beide
...
- **Reaktionszeile**: mindestens eine Zeile mit `machart` je Short, und
...
- **Bauform laufweit**: keine zweimal hintereinander (Fehler), ab sechs
  ...
  gescheitert ist
- Alle vier Regeln an manipulierten Shorts gegengeprüft: jede feuert, und
  der unveränderte `passwort-wechseln` bleibt still
- **`erstes-laden` umgeschrieben** — Wechselrede, vier Reaktionen in vier
...
- Fehlalarm der Zeitregel abgestellt: „empfiehlt heute, vorher zu
...
- **`ersatzteil-freischalten` umgeschrieben** — Wechselrede, vier
...
- **`video/Wortwechselprobe.tsx`** — der Prüftisch, der die Posenfrage
...
- **`raumstation-alte-rechner` ist der erste Short der Bauform
...
- **`npm run pruefen` ist wieder grün, ohne einen einzigen Hinweis** —
  zum ersten Mal seit der fordernden Regel
- In der `zahl`-Szene schrumpften die Figuren: Zahl und Bedeutung nahmen
...
- **`npm run laengen`** — Länge gegen Verweildauer. Anlass: `laengeSek`
  ...
  gestellt ist
- `Herkunft` trägt jetzt `bauform`, **optional**: Die Läufe vor dem
...
- **Der Längenversuch bis Oktober ist aufgesetzt.** Die Zielwerte sind
...
- `LAENGENKLASSEN` in `src/zeit.ts`, **aus `BAUFORMEN` abgeleitet**:
...
- Laufregel: Hinweis, wenn ab drei Shorts alle in derselben Klasse
  ...
  gegengeprüft
- `npm run laengen` zeigt jetzt auch „je Format", eindimensional. Format
  mal Länge wären zwölf Felder mit je drei Videos bis Oktober — Rauschen
- **Sprecherwechsel-Pausen zählen mit.** Es waren 1,2 bis 1,6 s je Short,
...
- Gerechnet wird **je Szene** (`zusatzpausenSzene`), nicht nur als Summe:
...
- **Wache über der Doppelung**: `npm run pruefen` hält `zusatzpausenSek`
...
- `npm run sprechprobe` rechnet die Pausen ebenfalls mit, sonst hätte die
  Spreizungswarnung eine fehlende Zeile als Textproblem gemeldet
- Regieanweisungen an die Macharten binden. Das war der Grund für den
  Wechsel auf `eleven_v3`. `regie` steht in `REAKTIONS_MACHARTEN`,
  `syntheseText` wählt daraus — **nur im Synthesetext**, `sprechtext`
  bleibt unberührt
- Wache über der Doppelung `Redeanteil.machart` ↔ `REAKTIONS_MACHARTEN`.
...
- Ein **Vorrat** statt eines festen Tags je Machart, gewählt
...
- Die sechs geratenen Tags wieder raus. `[confused]` stand seit dem 25.08.
...
- `npm run stimmprobe-v3` gelaufen (2.600 Zeichen). **Beide Messgrößen
...
- Daraus die Planänderung: **Das Sieb gehört ans Ende, nicht an den
...
- Die Reglerstufen gehört. **0,45 klingt ideal**, die Zahl bleibt — aber
  erstmals aus einem Vergleich statt aus der v2-Zeit. Der Kommentar an
  `KANAL_STIMME` sagt das jetzt auch
- Das Durchhören der 18 `sieb-*.mp3` **gestrichen**, und zwar als eigener
  ...
  gründlich aus
- `stability 0` gegen `1`: **deutlich verschieden.** Die Stimme reagiert
...
- Erster Lauf mit Ton am 30.08.2026, ohne Tags. **Zwei von vier
  ...
  jetzt `laufPruefen`
- `ZEICHEN_PRO_SEKUNDE` von 13,0 auf **14,3**, die im Kommentar
...
- `passwort-wechseln` auf die Bauform `zitatkarte`. Drei Wechselreden
...
- Aufschlag von `passwort-wechseln` gemessen 4,0 s bei höchstens 3,5.
...
- Drei verbotene Posen im Wortwechsel von `passwort-wechseln`
...
- Die Bauform `stationen` an einem echten Thema gebaut
- Bildtexte von `passwort-wechseln` an den neuen Sprechtext angeglichen:
...
### Nachtrag vom 25.08.2026 — Kanaltexte neu
...
- Neue Bio für alle drei Plattformen: „Technikfakten, die niemand
  nachliest. / Wir schon 📄"
- `marke-texte.md` nachgezogen: TikTok, Instagram, YouTube, Name-Feld,
  Kontakt, die überholte Takt-Regel

### Nachtrag vom 25.08.2026 — Banner auf der Handyansicht

- **Der Freiraum ist die Silhouette des Textblocks**, kein Rechteck um
  ...
  Telefon
- **Geprüft wird der Kasten eines Musterteils, nicht sein Ankerpunkt.**
...
- **Das Muster ist innen dicht und außen dünn.** Gleichverteilt lag der
  größte Teil dort, wo ihn niemand sieht: Auf dem Telefon zeigt YouTube nur
  1235 × 338 von 2048 × 1152
- **Zwei Markenbilder statt vier.** Das dunkle Profilbild und die
...
### Nachtrag vom 24.08.2026 — Stufe 6 · aufgeräumt

- **`laeufe/` von 127 auf 19 MB.** Die acht Videos vom 23.08. waren ein
...
- **`symbol` gestrichen**, das Feld für die stehende Zeichnung unter dem
...
- **Szenenart `hook` gestrichen.** Sie stand seit dem 17.08. als
...
### Nachtrag vom 24.08.2026 — Stufe 5 · Doku entdoppelt

- `rueckblick-lesen`: Die Rechnung „acht Formate, acht Videos die Woche"
...
- `thema-finden`: Materialgrenze und breitere Nische stehen im Vertrag,
  hier bleibt nur die Folge fürs Arbeiten. Der Ideenvorrat trägt jetzt den
  Hinweis, dass an ihm der Takt hängt
- CLAUDE.md: „je Sendeplatz" und die zweite Wortmarke im Nachschlag
  waren Reste aus zwei Umbauten

### Nachtrag vom 24.08.2026 — Takt festgelegt

- **Vier Videos je Woche.** Nicht geschätzt, sondern über den Engpass
...
### Nachtrag vom 24.08.2026 — Zweite Sichtung

- **Der Arm greift ins Symbol.** Die Symbolposition x = 138 war an der
...
- **Der Zeigestab ist weg** — samt `'stab'` im Schema, der Zeichnung, den
...
- **Der Spruch steht wieder in der Mitte**, unter dem Schlusssatz, mit
...
- **Die Signatur ist eine Geste geworden:** ein kurzer Strich von 96
...
- **Die Figur steht unter dem Spruch statt daneben**, am rechten Ende
...
- **Zwei neue Zeichnungen:** `satellit` statt `mond` für den Weltraum —
...
- **Die Figur steht rechts neben der Signatur**, größer, vertikal
...
### Nachtrag vom 24.08.2026 — Nachbesserung aus der ersten Sichtung
...
- **Zahlen stehen als Ziffer**, auch im Sprechtext. Der Untertitel hat es
...
- **Zielwert 30 s statt 23, Fenster 20–36 statt 18–34.** Der Zielwert war
...
- **Jeder gesprochene Satz hat ein Verb** (`voice.md`). Die alte Vorgabe
  „zwei bis sechs Wörter" und „Ellipsen sind die Stimme" hat den
  Telegrammstil nicht erlaubt, sondern erzwungen
- **Symbol und Figur standen beide bei x = 138.** Im fertigen Video lag
...
- **Die Figur konnte nicht lächeln.** Das Rig kannte vier Mundformen, und
...
- Vier Projekt-Skills entfernt: `design-critique`,
...
- Drei globale Symlinks entfernt: `ai-video-generation` (kostenpflichtig),
...
- **ffmpeg ohne Homebrew.** Der Rechner hat kein `brew`, aber Remotion
...
- Gegenprobe gegen ein echtes Video: 1080 × 1920, 30 fps, Video- und
  Tonstrom, 22,27 s
- Neuer Skill `bild-bauen` (184 Z.): rettet die übertragbaren
...
- `daten/marke/brand-profile.md` (171 Z.). Zielgruppe: 18–30,
...
- `daten/marke/voice.md` (173 Z.), aus 23 vertonten Sprechtexten. Anker
...
- Stimmtest gefahren, wie der Skill ihn verlangt. Er hat eine Signatur
...
- **Der Abo-Hinweis wird still eingeblendet, nie gesprochen und nie als
  Schlusssatz.** So kollidiert er nicht mit dem Rundlauf, auf dem der Kanal
  seine zweiten Ansichten holt
- Neue Quellenart **`wissenschaft`** als vierte unbeteiligte Art:
  ...
  draußen
- **Erste tote Regel gefunden und gestrichen:** `OFFIZIELLE_ARTEN` in
...
- `beleg-holen` nachgezogen, `npm run pruefen` grün

#### Stufe 2 · Formate neu

- **Aus acht Formaten wurden vier**, sortiert nach Reaktion:
  `gibtswirklich` (Staunen), `absicht` (Empörung), `eswareinmal`
  (Korrektur), `werhatrecht` (Widerspruch). `empfehlung` bleibt außerhalb
- `heimlich` in `absicht` aufgegangen. Die alte Abgrenzung „gebaut gegen
...
- `neu` und `auchgekauft` gestrichen. **Beide Verluste sind im Code
  benannt statt weggelassen** — `auchgekauft` war die Vorarbeit fürs
  Empfehlen und muss später anders erarbeitet werden
- **`dubistdumm` ist keine Themengruppe, sondern eine Machart.** Das
  ...
  je Video verlangt
- **Der Wochentag ist weg.** `FORMATE[...].tag` gestrichen,
  ...
  nicht gibt
- **Die Formatregel hat die Richtung gewechselt:** statt „jedes Format
  ...
  verlangt
- Gegentest über vier Fälle gefahren: zwei gleiche hintereinander →
  ...
  nirgends mehr an
- **77 Ideen umsortiert, keine verloren** — `gibtswirklich` 21,
  ...
  mitgewandert
- Zwei neue Sachgebiete `raumfahrt` und `zeit` — und gleich mit Inhalt
...
- CLAUDE.md nachgezogen: Formattabelle, Sachgebiete, Quellenrang, Takt,
...
- `thema-finden` neu geschrieben: Humor und Rundlauf raus (stehen jetzt
  bei `FORMATE` und in `voice.md`), Formattabelle und Materialgrenze rein

#### Stufe 3–6 · danach

- Stufe 3a: Figur geriggt — `src/figur.ts` (Vertrag), `daten/figur/`
...
- Stufe 3b: Längenfenster auf 18–34 s, `zaehlung` als Szenenfeld mit
...
- Stufe 6: aufräumen — teilweise. Was übrig ist, steht in der Liste am
  Ende: `laeufe/` ist inzwischen 235 MB in einem einzigen Ordner

### Nachtrag vom 19.08.2026 — Der Rücklauf wird lesbar
...
- `src/rueckschau.ts` — legt Rückblick und Herkunft zusammen. Die Brücke
  ist `lauf.json`, nicht `verlauf.json`: Der Verlauf kennt nur die
  `themaId`, der Rückblick nur die `shortId`
- Zwei Messungen je Short statt einer. `mitHalt` neben `zuletzt`, weil
  Analytics ein bis drei Tage nachhinkt — sonst liest man `null` und hält
  ein Video für tot, obwohl gestern eine Zahl dastand
- `npm run ausreisser` — Haltequote neben Format und Thema, Ausreißer mit
  `▲`, Aufrufe ganz rechts
- `npm run aufschlaege` — jeder Aufschlag neben seiner Haltequote. Das
  Gegenstück zu `npm run belege`: stellt nebeneinander, urteilt nicht
- **Beide schweigen, wenn sie nichts wissen.** Median erst ab acht
  ...
  `0 %`
- Join gegen `daten/verlauf.json` als unabhängige Quelle gegengeprüft —
  beide Shorts stimmen in Format, Sachgebiet und Thema überein
- Zwei Sprachfehler in der eigenen Ausgabe behoben: „1 Tage draußen" und
  „2 Aufschlag/Aufschläge". `tageText` für die Tabellenspalte (Nominativ),
  `seitText` für den Satz (Dativ)
- `npm run pruefen` sagt den Rücklauf in einem Satz — an der Stelle, an
...
- Gegentest über sechs Randfälle: fehlende Datei, fehlender Ordner,
...
### Nachtrag vom 19.08.2026 — Neues Fundament
...
- Vier Skills unter `.claude/skills/`: `beleg-holen` (111 Z.),
  `woche-bauen` (107 Z.), `thema-finden` (114 Z.), `rueckblick-lesen`
  (87 Z.)
- Subagent `.claude/agents/belegpruefer.md` — liest die Zitat-Paare in
  eigenem Kontext und meldet nur, wo ein Satz mehr behauptet als sein
  Zitat trägt

#### CLAUDE.md gekürzt · 19.08.2026

- **928 → 713 Zeilen.** Neun Abschnitte in die vier Skills ausgelagert,
...
- Neuer Abschnitt **„Wo was steht"** ganz vorn: eine Tabelle der vier
  Skills und des Subagenten. Diese Datei hält den Vertrag, die Skills
  halten die Abläufe
- **Ein Fehler im Skill gefunden, bevor das Original gelöscht wurde:**
...
- Sechs Begründungen nachgetragen, die in keinem Skill standen:
...
- Stop-Hook in `.claude/settings.local.json`: `npm run pruefen` nach jedem
  Zug, still bei Erfolg, `systemMessage` mit den letzten 15 Zeilen bei
  Rot. Er meldet, er blockiert nicht
- **Stop und nicht PostToolUse:** `tsc` läuft über das ganze Projekt —
  ...
  rot ist
- Beide Fälle vor dem Eintragen einzeln gefahren, der Fehlerfall mit
...
- Stufe 2: `LAENGE_SEK.ziel` von `[18, 28]` auf `[18, 30]` — **überholt**.
  Das Fenster steht heute bei 42–67 s mit vier Zielwerten je Bauform
- Stufe 3.1: Kamera-Layer und Übergänge — **verworfen**, gerechnet:
...
- Stufe 4: `EINGEFROREN.md` in `~/Desktop/horizont-bench` — der Bench
...
### Nachtrag vom 18.08.2026 — Was damals offen war

*Was hier offen war, steht zusammengeführt in **Offen · Stand 31.08.2026** am Ende dieser Datei.*

- **Vorschaubild bei TikTok: nichts zu machen.** Buffer schreibt es ins
...
### Nachtrag vom 18.08.2026 — Bebilderung
...
- Symbolvorrat von 8 auf **38**; 34 von 38 Szenen bebildert
- Untergrenze für die Zeichnung: Sie bekam nur den Rest der Bühne und kam
  in textreichen Szenen mit 60 statt 560 Pixeln an
- **Doktrin korrigiert.** „Keine Gerätezeichnungen" war als „keine
  ...
  Satz nennt
- Drei Zeichnungen im Standbild verworfen: Ladenkasse (las sich wie der
  Fernseher), zwei gespiegelte Kabel (lasen sich als Ohrhörer), Waage für
  „zwei Kabel"
- `--ton-behalten` parste die alten Renderdaten gegen das **aktuelle**
...
- `--ton-behalten` findet den Ton auch in einem früheren Lauf. Vorher
  brach es um Mitternacht ab, weil der Tagesordner wechselte

### Nachtrag vom 17.08.2026 — Was danach anstand

- `@ganzakkurat` auf allen drei Plattformen gesichert und umbenannt
- `ganzakkurat.de` bei STRATO als Kontodomain hinterlegt
- **Keine eigene Domain für die Videoablage.** Die `pub-…r2.dev`-Adresse
  ...
  Verhältnis
- `kontakt@ganzakkurat.de` bei STRATO angelegt

### Nachtrag vom 17.08.2026 — Umzug auf „Ganz akkurat"

- Buffer auf die drei neuen Kanäle umgestellt — alle drei verbunden,
  keiner getrennt
- Cloudflare: `ganzakkuratvideos` angelegt und öffentlich freigegeben,
  `.env` umgestellt, alle Zugänge mit echtem Hin-und-Rück geprüft
- Die zwei letzten SetupKlar-Videos gelöscht, `setupklarvideos` entfernt
- Aufräumregel: Objekte verschwinden nach 90 Tagen von selbst
- GitHub angelegt und gepusht. Historie vorher vereinheitlicht: ein
  Autor statt zwei, acht alte Banner raus, 35 MB → 6,3 MB
- `.gitignore` deckt jetzt alle `.env`-Varianten ab. Eine Sicherungskopie
  lag daneben, die keine Einzelregel traf — eine Aufzählung kann man
  vergessen, ein Muster nicht
- Letzte SetupKlar-Spuren im Code: veraltetes Beispielkürzel im
  Wochenlauf. Die historischen Kommentare bleiben stehen, sie erklären,
  warum etwas so ist

### Nachtrag vom 17.08.2026 — Der Mittwoch ist automatisiert
...
- SPARQL-Abfrage auf `resource_legal_date_entry-into-force`, Volltexte
  über denselben Cellar-Weg wie `quellen-pruefen`
- **Gegenprobe bestanden:** Der erste scharfe Lauf setzte die
...
- Drei Anläufe, zwei Befunde im Skript festgehalten: 83 Kandidaten waren
...
### Nachtrag vom 17.08.2026 — Entschieden und erledigt

- Bilder hochgeladen, Bios eingetragen, Kanäle stehen auf allen drei
  Plattformen
- Die Videos sehen auf den Plattformen richtig aus — geprüft am
  Montagsvideo
- Rechner neu gestartet
- `emirhanokyay@hotmail.com` im GitHub-Konto eingetragen
- **Kleinschreibung des YouTube-Handles bleibt, wie sie ist.** `@Ganzakkurat`
...
- **Der Sendeplan kommt nicht aufs Banner zurück.** Entschieden, nicht
  ...
  außen
- **Kein Anwaltstermin.** Die drei Fragen — Werbekennzeichnung im Video,
...
### Nachtrag vom 17.08.2026 — Der Kanal läuft

- Alle acht Shorts abgenommen. Der Dienstag ging dreimal zurück: erst
...
- **`npm run veroeffentlichen` erstmals komplett gelaufen** — 24 Beiträge
  auf drei Kanälen, Mo 17.08. bis So 23.08. Kein Fehlschlag
- `--ab=<datum>` gebaut: Die Woche beginnt am Tag, an dem sie fertig
  wurde, nicht sieben Tage später
- Vergangene Termine gehen gleich raus statt still zu scheitern,
  gestaffelt, damit nicht zwei auf dieselbe Minute fallen
- ElevenLabs bekommt drei Versuche; ein leerer Rumpf mit HTTP 200 heißt
  jetzt auch so und nicht „Unexpected end of JSON input"
- Render-Wartezeit auf zwei Minuten — 25 Sekunden reichen für einen
  kalten Chrome-Start nicht, wenn nebenher etwas läuft
- Markenbilder aus dem Code erzeugt (`npm run markenbilder`): Profilbild
  hell und dunkel, YouTube-Banner, freigestellte Wortmarke
- Kanaltexte in `marke-texte.md`, ohne Taktzusage

### Nachtrag vom 17.08.2026 — Belege strukturell abgesichert
...
- Jeder Beleg in `quellen.json` hat ein `id` (50 Zitate)
- Jede Szene mit `quelleId` trägt eine `belegId` — Schema erzwingt das
  Paar, `shortPruefen` prüft, dass die Fundstelle wirklich dort steht
- Hinweis, sobald ein Zitat mehr als zwei Szenen tragen soll — das
  Muster, hinter dem sich alle drei Fehler versteckt hatten
- Belegansicht zeigt nur noch das gebundene Zitat: 82 Paare → 32
- **Dienstag**: Das Märchen sitzt jetzt ganz im Aufschlag, der belegfrei
...
- **Mittwoch 18 Uhr**: „Kein Zufall. Ein Gremium hat das so festgelegt."
  war eine Behauptung über eine Absicht. Jetzt: was das Gremium wirklich
  festgelegt hat — steht wörtlich da und ärgert genauso
- **Donnerstag**: „Neunundzwanzig Euro" waren erfunden. Raus
- **Montag**: „In der Zeit wurde ein Kind eingeschult" hatte keine
  Fundstelle. Ersetzt durch das belegte Ende der Geschichte
- **Sonntag**: Die Prämisse „abends" hing an nichts — die
  Bundesnetzagentur schreibt über das Band, nicht über die Uhrzeit
- Gegentest gefahren: alle drei neuen Regeln greifen
- Beim Lesen der 32 Paare vier weitere Sätze gefunden, die mehr sagten
...
- Alle acht neu vertont und gerendert: **0 Fehler, 0 Hinweise**,
  19,5–22,5 s, 2.828 Zeichen Kontingent
- Vorrat aufgefüllt: 56 → **77 Ideen**, Reichweite 5 → **8 Wochen**.
...
### Nachtrag vom 17.08.2026 — Der große Umbau
...
- Zweite Person erlaubt, der Sprecher ist mitgemeint
- Beleg wird eingeblendet statt gespielt — die 2,5 s gehen in die Pointe
- 56 erzählbare Themen gesammelt, 11 gestrichen, 45 bleiben
- Formate aus dem Material abgeleitet statt umgekehrt; Wochentage neu
- Montag heißt **„Du bist dumm"** — der Spott steht im Namen

#### Gebaut am 17.08.2026

- `src/typen.ts` — `Position` als Pflichtfeld, sieben statt zehn Szenenarten
- Sieben neue Sendeplätze, acht neue Sachgebiete, `MATRIX` neu
- `merksatz` → `weitererzaehlt`, `kernfrage` → `erzaehlt`, `system` gestrichen
- Belegeinblendung unter der Kopfzeile (`Belegzeile` in `Wortmarke.tsx`)
- `frage`-Szene mit Denkpause über den Szenentrenner — kostet kein Kontingent
- `schluss` ersetzt die Endkarte: ein Satz statt einer Liste
- Gerätezeichnungen und `src/illustration.ts` gestrichen, Typografie trägt
- Zwei Fehlalarme abgestellt: `bildvielfalt` und „vom" als Sachwort
- Drei Quellen neu abgerufen und maschinell bestätigt (UBA, BSI × 2)
- Sieben neue Shorts, **0 Fehler, 0 Hinweise**, 22,8–25,4 s
- Neuer Ideenvorrat: 42 Ideen, eine Datei je Sendeplatz
- Standbilder geprüft: Frage, Belegzeile, Nachschlag, Vergleich
