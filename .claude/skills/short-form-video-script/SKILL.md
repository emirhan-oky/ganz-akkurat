...
description: >-
  ...
  variants + CTA); the human shoots/performs/edits; Buffer publishes the finished file. Routes
...
the **human** shoots it, **Buffer publishes** the finished file.
...
- **The agent** writes the script (spoken + on-screen text + beat/shot direction + hook variants + CTA); the
  **human** shoots/performs/edits/decides the take; **Buffer publishes** the finished file (measurement: the platforms' native
...
the platform script skill's output → **Buffer.** Measure with: native +
...
Buffer publishes the finished file; measured on 3s hold / AVD / replays / saves rather than likes;
